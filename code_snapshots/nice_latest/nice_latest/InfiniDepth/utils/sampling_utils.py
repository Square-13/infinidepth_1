...
def make_3d_uniform_coord_triangle(
    ...
    if triangle_internal_guard and len(faces) > 0:
        ...
        guarded_faces = depth_risk | rgb_risk
    ...
    if deterministic:
        ...
        face_i = i_centroids
        face_j = j_centroids
        if triangle_vertex_choice is not None and guarded_faces.any():
            face_i = i_centroids.clone()
            face_j = j_centroids.clone()
            guarded_idx = torch.nonzero(guarded_faces, as_tuple=True)[0]
            chosen = triangle_vertex_choice[guarded_idx].long().clamp(0, 2)
            face_i[guarded_idx] = fi_all[guarded_idx, chosen]
            face_j[guarded_idx] = fj_all[guarded_idx, chosen]
