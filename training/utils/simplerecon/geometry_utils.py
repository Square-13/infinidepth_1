...
class BackprojectDepth(jit.ScriptModule):
    ...
    def __init__(self, height: int, width: int):
        ...
        pix_coords_2hw = torch.stack((xx, yy), axis=0) + 0.5  # (2, H, W)

        pix_coords_13N = to_homogeneous(
                                pix_coords_2hw,
                                dim=0,
                            ).flatten(1).unsqueeze(0)  # (1, 3, H*W)
    ...
    def forward(self, depth_b1hw: Tensor, invK_b44: Tensor) -> Tensor:
        ...
        """
        cam_points_b3N = torch.matmul(invK_b44[:, :3, :3], self.pix_coords_13N)  # (B, 3, H*W)
        cam_points_b3N = depth_b1hw.flatten(start_dim=2) * cam_points_b3N  # (B, 3, H*W)
        cam_points_b4N = to_homogeneous(cam_points_b3N, dim=1)  # (B, 4, H*W)
...
class Project3D(jit.ScriptModule):
    ...
    def forward(self, points_b4N: Tensor,
        ...
        """
        P_b44 = K_b44 @ cam_T_world_b44  # (B, 4, 4)

        cam_points_b3N = P_b44[:, :3] @ points_b4N  # (B, 3, H*W)
        ...
        mask = torch.abs(cam_points_b3N[:, 2:]) > self.eps  # (B, 1, H*W)
        depth_b1N = (cam_points_b3N[:, 2:] + self.eps)  # (B, 1, H*W)
        scale = torch.where(mask, 1.0 / depth_b1N, torch.tensor(1.0, device=depth_b1N.device))  # (B, 1, H*W)
        
        pix_coords_b2N = cam_points_b3N[:, :2] * scale  # (B, 2, H*W)
        
        return torch.cat([pix_coords_b2N, depth_b1N], dim=1)  # (B, 3, H*W)
...
def pose_distance(pose_b44):
    ...
    R = pose_b44[:, :3, :3]  # (B, 3, 3)
    t = pose_b44[:, :3, 3]  # (B, 3)
    R_trace = R.diagonal(offset=0, dim1=-1, dim2=-2).sum(-1)  # (B)
    R_measure = torch.sqrt(2 * 
                (1 - torch.minimum(torch.ones_like(R_trace)*3.0, R_trace) / 3))  # (B)
    t_measure = torch.norm(t, dim=1)  # (B)
    combined_measure = torch.sqrt(t_measure ** 2 + R_measure ** 2)  # (B)
