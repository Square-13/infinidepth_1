...
from .generic_utils import combine_dims, tensor_B_to_bM, tensor_bM_to_B
from .geometry_utils import (BackprojectDepth, Project3D, get_camera_rays,
                                  pose_distance)
...
class CostVolumeManager(nn.Module):
    ...
    def initialise_for_projection(self):
        ...
        linear_ramp = torch.linspace(0, 1, 
                        self.num_depth_bins).view(1, self.num_depth_bins, 1, 1)  # (1, D, 1, 1)
    ...
    def generate_depth_planes(self, batch_size: int, 
        ...
        log_depth_planes_bd11 = (torch.log(min_depth) + 
                                    torch.log(max_depth / min_depth) 
                                        * linear_ramp_bd11)  # (B, D, 1, 1)
        depth_planes_bd11 = torch.exp(log_depth_planes_bd11)  # (B, D, 1, 1)
    ...
    def warp_features(
        ...
        world_points_b4N = self.backprojector(depth_plane_b1hw, cur_invK)  # (B, 4, H*W)
        ...
        cam_points_B3hw = cam_points_B3N.view(-1, 3, self.matching_height, 
                                                            self.matching_width)  # (B*S, 3, H, W)
        pix_coords_B2hw = cam_points_B3hw[:, :2]  # (B*S, 2, H, W)
        depths = cam_points_B3hw[:, 2:]  # (B*S, 1, H, W)

        uv_coords = 2 * pix_coords_B2hw.permute(0, 2, 3, 1) * uv_scale - 1  # (B*S, H, W, 2)
        ...
        src_feat_warped = src_feat_warped.reshape(
                                            batch_size,
                                            num_src_frames,
                                            num_feat_channels,
                                            self.matching_height,
        ...
        mask_b = depths > 0  # (B, S, H, W)
        mask = mask_b.type_as(src_feat_warped)  # (B, S, H, W)
    ...
    def build_cost_volume(
        ...
        for depth_id in range(self.num_depth_bins):
            ...
            depth_plane_b1hw = depth_planes_bdhw[:, depth_id].unsqueeze(1)  # (B, 1, H, W)
            ...
            # Sum over the frames
            dot_product_b1hw = dot_product_bkhw.sum(dim=1, keepdim=True)  # (B, 1, H, W)

            all_dps.append(dot_product_b1hw)

        cost_volume = torch.cat(all_dps, dim=1)  # (B, D, H, W)
...
class FeatureVolumeManager(CostVolumeManager):
    ...
    def __init__(self, 
                ...
                matching_dim_size = 4,  # TODO: VAE encoded visual feature channel
                num_source_views = 2):
        """
        Args:
            matching_height: height of input feature maps
    ...
    def build_cost_volume(self, 
        ...
        frame_pose_dist_B, r_measure_B, t_measure_B = pose_distance(
                                                tensor_bM_to_B(src_poses)  # (B*S, 4, 4)
                                            )  # (B*S), (B*S), (B*S)
        ...
        r_measure_bkhw = tensor_B_to_bM(
                                ...
                                batch_size=batch_size, 
                                num_views=num_src_frames
                            )[:,:,None,None].expand(frame_pose_dist_bkhw.shape)  # (B, S, H, W)
        ...
        for depth_id in range(self.num_depth_bins):
            ...
            world_points_B4N = world_points_b4N.repeat_interleave(
                                                        num_src_frames, dim=0)  # (B*S, 4, H*W)
            ...
            if return_mask:
                ...
                pix_coords_bk2hw = pix_coords_B2hw.reshape(
                                                    batch_size,
                                                    num_src_frames,
                                                    2,
                                                    self.matching_height,
                ...
                bounds_mask = torch.any(self.get_mask(pix_coords_bk2hw), dim=1)  # (B, H, W)
                overall_mask_bhw = torch.logical_and(depth_mask, bounds_mask)  # (B, H, W)
            ...
            cur_points_rays_bk3hw = tensor_B_to_bM(cur_points_rays_B3hw, 
                                batch_size=batch_size, num_views=num_src_frames)  # (B, S, 3, H, W)
            
            # compute rays for world points source frame 
            src_poses_B44 = tensor_bM_to_B(src_poses)  # (B*S, 4, 4)
            ...
            # run through the MLP!
            mlp_input_features_bhwc = mlp_input_features_bchw.permute(0,2,3,1)  # (B, H, W, Cm)
            feature_b1hw = self.mlp(
                                mlp_input_features_bhwc
                            ).squeeze(-1).unsqueeze(1)  # (B, H, W, 1) -> (B, 1, H, W)

            # append MLP output to the final cost volume output.
            all_dps.append(feature_b1hw)

        feature_volume = torch.cat(all_dps, dim=1)  # (B, D, H, W)

        return feature_volume, depth_planes_bdhw, overall_mask_bhw  # (B, D, H, W), (B, D, H, W), (B, H, W)
