...
_data_type_reverse = {b: a for (a, b) in _data_type_relation}
...
def _lookup_type(type_str):
    if type_str not in _data_type_reverse:
        try:
            ...
            raise ValueError(f"field type {type_str!r} not in {_types_list!r}")
...
class PlyData:
    ...
    def _index(self):
        self._element_lookup = {elt.name: elt for elt in self._elements}
...
class PlyElement:
    ...
    def _index(self):
        self._property_lookup = {prop.name: prop for prop in self._properties}
...
class PlyListProperty(PlyProperty):
    ...
    def __repr__(self):
        return "PlyListProperty({!r}, {!r}, {!r})".format(
            self.name,
